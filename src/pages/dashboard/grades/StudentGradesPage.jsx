        ...
        <div className="space-y-6">
            ...
            <Card>
                <Table headers={['Curso', 'Créditos', 'Parcial', 'Final', 'Promedio', 'Estado']}>
                    {loading ? (
                        <TableRow><TableCell colSpan="6" className="text-center">Cargando notas...</TableCell></TableRow>
                    ) : enrollments.length === 0 ? (
                        <TableRow><TableCell colSpan="6" className="text-center">No estás matriculado en cursos este periodo</TableCell></TableRow>
                    ) : (
                        enrollments.map((course, idx) => {
                            ...
                            return (
                                <TableRow key={idx}>
                                    <TableCell><span className="font-semibold text-slate-700">{course.nombre_curso || `Curso ${idx + 1}`}</span></TableCell>
                                    <TableCell>3.0</TableCell>
                                    <TableCell>{course.parcial}</TableCell>
                                    <TableCell>{course.final}</TableCell>
                                    <TableCell><span className={`font-bold ${avg >= 11 ? 'text-blue-600' : 'text-red-500'}`}>{avg}</span></TableCell>
                                    <TableCell>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                 <Card title="Promedio Ponderado" className="bg-gradient-to-br from-primary-600 to-primary-800 text-white">
                 ...
                 <Card title="Créditos Aprobados">
                     <div className="text-4xl font-bold text-center text-slate-700 py-4">
                         18 / 22
                     </div>
                 </Card>
                 <Card title="Asistencia General">
                     <div className="text-4xl font-bold text-center text-emerald-600 py-4">
                         92%
                     </div>
                 </Card>
            </div>
