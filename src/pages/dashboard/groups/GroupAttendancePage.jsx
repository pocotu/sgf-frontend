...
const GroupAttendancePage = () => {
  ...
  if (loading) {
    return <div className="p-8 text-center text-slate-500">Cargando lista...</div>;
  }
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          ...
          <p className="text-slate-500">
            Grupo: {group?.nombre_grupo} • {group?.modalidad}
          </p>
        </div>
              ...
              <TableCell>
                <div className="flex items-center gap-4">
                  <label className="flex items-center gap-2 cursor-pointer">
                    ...
                    />
                    <span
                      className={`text-sm ${attendanceData[student.estudiante_id] === 'PRESENTE' ? 'font-bold text-emerald-700' : 'text-slate-600'}`}
                    >
                      Presente
                    </span>
                  </label>
                  ...
                  <label className="flex items-center gap-2 cursor-pointer">
                    ...
                    />
                    <span
                      className={`text-sm ${attendanceData[student.estudiante_id] === 'TARDANZA' ? 'font-bold text-orange-700' : 'text-slate-600'}`}
                    >
                      Tardanza
                    </span>
                  </label>
                  ...
                  <label className="flex items-center gap-2 cursor-pointer">
                    ...
                    />
                    <span
                      className={`text-sm ${attendanceData[student.estudiante_id] === 'AUSENTE' ? 'font-bold text-red-700' : 'text-slate-600'}`}
                    >
                      Ausente
                    </span>
                  </label>
