        ...
        <div className="space-y-6">
            ...
            <Card>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                    {/* 1. Select Group */}
                    <div>
                        <label className="block text-sm font-semibold text-slate-700 mb-2">1. Seleccionar Grupo</label>
                        <select 
                        ...
                        >
                            ...
                            {groups.map(g => (
                                <option key={g.grupo_id} value={g.grupo_id}>{g.nombre_grupo} - {g.nombre_curso || 'Curso'}</option>
                            ))}
                        </select>
                    ...
                    <div>
                        <label className={`block text-sm font-semibold mb-2 ${!selectedGroup ? 'text-slate-400' : 'text-slate-700'}`}>
                        ...
                        >
                            <option value="">
                                {evaluations.length === 0 && selectedGroup ? "No hay evaluaciones" : "-- Seleccione Evaluación --"}
                            </option>
                            {evaluations.map(ev => (
                                <option key={ev.evaluacion_id} value={ev.evaluacion_id}>{ev.nombre}</option>
                            ))}
                        </select>
                        {loadingEvaluations && <p className="text-xs text-slate-400 mt-1">Buscando evaluaciones...</p>}
                    </div>
                </div>
                ...
                {selectedGroup && selectedEvaluation && (
                    <div className="animate-in fade-in slide-in-from-bottom-4 duration-500">
                        <div className="flex items-center justify-between mb-4">
                             <h3 className="font-semibold text-slate-800">Listado de Estudiantes ({students.length})</h3>
                        </div>

                        {loadingStudents ? (
                             <div className="text-center py-8 text-slate-500">Cargando lista de estudiantes...</div>
                        ) : students.length > 0 ? (
                            <>
                                <Table headers={['Estudiante', 'Código', 'Nota (0-20)']}>
                                    {students.map(student => (
                                        <TableRow key={student.estudiante_id}>
                                            <TableCell>
                                                <div className="font-medium text-slate-900">{student.nombre_estudiante}</div>
                                            </TableCell>
                                            <TableCell>
                                                <span className="font-mono text-xs text-slate-500">{student.codigo_interno}</span>
                                            </TableCell>
                                            <TableCell>
                                                <input 
                                                    type="number" 
                                ...
                                <div className="mt-6 flex justify-end gap-3">
                                    <Button variant="outline" onClick={() => setSelectedEvaluation('')}>Cancelar</Button>
