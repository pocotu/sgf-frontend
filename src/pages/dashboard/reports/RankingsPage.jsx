    ...
    <div className="space-y-6">
       ...
       <Card>
           <Table headers={['Puesto', 'Estudiante', 'Facultad', 'Promedio Ponderado']}>
               {loading ? (
                   <TableRow><TableCell colSpan="4" className="text-center">Cargando rankings...</TableCell></TableRow>
               ) : (
                   rankings.map(r => (
                       <TableRow key={r.estudiante_id}>
                           <TableCell>
                               <span className={`font-bold ${r.puesto <= 3 ? 'text-primary-600 text-lg' : 'text-slate-500'}`}>#{r.puesto}</span>
                           </TableCell>
                           <TableCell><span className="font-medium text-slate-800">{r.nombre}</span></TableCell>
                           <TableCell>{r.facultad}</TableCell>
                           <TableCell><Badge variant="primary">{r.promedio}</Badge></TableCell>
                       </TableRow>
                   ))
               )}
           </Table>
